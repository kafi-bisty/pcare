...
// লগইন চেক (নিশ্চিত করুন যে এটি অ্যাডমিন প্যানেলের অনুমোদিত ইউজার)
...
// ভর্তি থাকা রোগী (IPD) এবং ইনবক্স
...
// সাম্প্রতিক অ্যাক্টিভিটি লগ (শুধুমাত্র এডমিন ও ম্যানেজার দেখবে)
...
<head>
    ...
    <title>ড্যাশবোর্ড | পেশেন্ট কেয়ার হাসপাতাল</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap/5.3.0/css/bootstrap.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        :root { --navy: #0A2647; --cyan: #2AA7E5; --dark-navy: #06182c; --light-gray: #f1f5f9; }
        body { background-color: var(--light-gray); font-family: 'Segoe UI', sans-serif; overflow-x: hidden; }
        
        /* সাইডবার ডিজাইন */
        .sidebar { height: 100vh; width: 260px; position: fixed; background: var(--navy); color: white; z-index: 1000; transition: 0.3s; }
        .sidebar-logo { width: 55px; height: 55px; border-radius: 50%; border: 2px solid var(--cyan); background: #fff; padding: 2px; }
        ...
        .stat-card { border: none; border-radius: 20px; transition: 0.3s; position: relative; overflow: hidden; color: white; height: 120px; display: flex; flex-direction: column; justify-content: center; }
        ...
        .tool-box:hover { background: var(--navy); transform: translateY(-8px); box-shadow: 0 15px 30px rgba(0,0,0,0.1); }
        .tool-box:hover h6, .tool-box:hover small { color: #fff !important; }
        .tool-icon { font-size: 2.5rem; margin-bottom: 12px; transition: 0.3s; }
        .tool-box:hover .tool-icon { transform: scale(1.1); color: var(--cyan) !important; }

        /* ফুটার স্টাইল */
        .footer { border-top: 1px solid #e2e8f0; margin-top: auto; }
...
<!-- সাইডবার শুরু -->
<nav class="sidebar shadow no-print">
    ...
    <div class="sidebar-menu">
        ...
        <!-- রোল ভিত্তিক কন্ট্রোল -->
        <?php if(in_array($user_role, ['admin', 'manager'])): ?>
            <div class="px-3 mt-4 mb-2 small text-muted text-uppercase" style="font-size: 10px;">Management</div>
...
<div class="main-content">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted small mb-0">আসসালামু আলাইকুম, <strong><?php echo $_SESSION['user_name']; ?></strong>!</p>
        </div>
    ...
    <div class="row g-3 mb-4 text-center">
        ...
        <div class="col-md-3">
            <div class="card stat-card bg-grad-cyan p-3">
                <small class="opacity-75">মোট স্টাফ মেম্বার</small>
                <h3 class="fw-bold mb-0"><?php echo $total_staff; ?> জন</h3>
                <i class="fas fa-users position-absolute bottom-0 end-0 m-3 opacity-25 fa-2x"></i>
            </div>
        </div>
    </div>

    <div class="row g-4 mb-5">
        <!-- বাম পাশে: অ্যাক্টিভিটি লগ (Admin/Manager দেখবে) -->
        <?php if(in_array($user_role, ['admin', 'manager'])): ?>
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm rounded-4 p-4 h-100">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h5 class="fw-bold text-navy mb-0"><i class="fas fa-history me-2 text-primary"></i>সাম্প্রতিক কার্যক্রম</h5>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="py-3 border-bottom border-light">
                                    <p class="mb-0 small fw-bold text-dark"><?php echo $log['user_name']; ?> <span class="fw-normal text-muted"><?php echo $log['details']; ?></span></p>
                                </td>
        ...
        <?php else: ?>
            <!-- অ্যাকাউন্টস রোলের জন্য বিশাল ব্যানার -->
            <div class="col-lg-8">
                <div class="card border-0 shadow-sm rounded-4 p-5 text-center bg-white h-100 d-flex flex-column justify-content-center">
                    <i class="fas fa-calculator fa-5x text-light mb-4"></i>
                    <h3 class="text-navy fw-bold">হিসাবরক্ষণ প্যানেল</h3>
                    <p class="text-muted">আপনার জন্য নির্ধারিত বিলিং এবং অ্যাকাউন্টস টুলসগুলো মেনু থেকে ব্যবহার করুন।</p>
                </div>
            </div>
        <?php endif; ?>

        <!-- ডান পাশে: কুইক শর্টকাট বাটন -->
        <div class="col-lg-4">
            <div class="card border-0 shadow-sm rounded-4 p-4 h-100">
                <h5 class="fw-bold text-navy mb-4">নতুন এন্ট্রি / সার্ভিস</h5>
    ...
    <!-- ম্যানেজমেন্ট গ্রিড -->
    <h4 class="fw-bold text-navy mb-4">ম্যানেজমেন্ট কন্ট্রোল গ্রিড</h4>
    <div class="row g-4 mb-5">
        <div class="col-md-4 col-lg-3">
            <a href="manage-accounts.php" class="tool-box shadow-sm">
        ...
        <?php if(in_array($user_role, ['admin', 'manager'])): ?>
            <div class="col-md-4 col-lg-3">
                <a href="manage-all-staff.php" class="tool-box shadow-sm">
                    <i class="fas fa-user-md tool-icon text-warning"></i>
                    <h6 class="fw-bold text-navy mb-1">Staffs</h6>
                    <small class="text-muted">স্টাফ লিস্ট ও এডিট</small>
                </a>
            </div>
            <div class="col-md-4 col-lg-3">
                <a href="manage-lab-tests.php" class="tool-box shadow-sm">
                    <i class="fas fa-vial tool-icon text-danger"></i>
                    <h6 class="fw-bold text-navy mb-1">Lab Tests</h6>
                    <small class="text-muted">টেস্টের তালিকা ও দাম</small>
                </a>
            </div>
            <div class="col-md-4 col-lg-3">
                <a href="messages.php" class="tool-box shadow-sm">
                    <i class="fas fa-envelope-open-text tool-icon text-info"></i>
                    <h6 class="fw-bold text-navy mb-1">Inbox (<?php echo $new_messages; ?>)</h6>
    ...
    <!-- ★ প্রফেশনাল ফুটার সেকশন ★ -->
    <footer class="footer mt-auto py-3 shadow-sm rounded-4 bg-white border no-print">
        <div class="container-fluid">
            <div class="row align-items-center">
                <div class="col-md-6 text-center text-md-start">
                    <span class="text-muted small">© <?php echo date('Y'); ?> <strong>পেশেন্ট কেয়ার হাসপাতাল</strong>। সর্বস্বত্ব সংরক্ষিত।</span>
                </div>
                <div class="col-md-6 text-center text-md-end">
                    <span class="badge bg-light text-navy border fw-normal">সিস্টেম ভার্সন ৩.০</span>
                    <span class="ms-2 small text-muted"><i class="fas fa-shield-alt text-success me-1"></i>Secure Data Control</span>
                </div>
            </div>
        </div>
    </footer>
